...
//修改后的
//下拉刷新用的
@property (strong, nonatomic) UIImageView *freshTopImageV;
@end
...
-(UIImageView *)freshTopImageV
{
    if(!_freshTopImageV){
        UIImageView *freshTopImageV = [[UIImageView alloc]init];
        freshTopImageV.contentMode = UIViewContentModeScaleAspectFit;
        freshTopImageV.image = [UIImage imageNamed:@"word_scroll.png"];
        [self addSubview:_freshTopImageV=freshTopImageV];
    }
    return _freshTopImageV;
}
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [ColorUtility colorWithHexString:@"f0f0f0"];
...
- (void)layoutSubviews
{
    ...
    float width_arraowImageV = 20;
    float width_worldLabel = 80;
    float leftSide = (self.frame.size.width-width_arraowImageV-width_worldLabel-10)/2.0;
    self.statusLabel.frame = CGRectMake(leftSide+width_arraowImageV, 72, width_worldLabel, width_arraowImageV);
    //
    [self.freshTopImageV setFrame:CGRectMake(40, 20, self.frame.size.width-80, 20)];
}
