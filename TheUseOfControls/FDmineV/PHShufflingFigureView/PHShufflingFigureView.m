...
-(void)dataSourceWithImageArray:(NSArray *)imageArray andDelegate:(id)delegate andTimeInterval:(float)timeInterval
{
    ...
    if([imageArray count]!=0)
    {
        ...
        {
            //启用计时器
            [self createPageControlWihImageArray:imageArray];
            [self startTimer];
        }
    }
}
#pragma mark -- 自定义pageControl
-(void)createPageControlWihImageArray:(NSArray *)imageArray
{
    //indicatorMargin设置间距
    float bottom_spacePageControl7 = 0;
    float height_spacePageControl7 = 30;
    float margin = 3.0f;//水平间距
    float diameter = 10.0f;//直径
    float width_spacePageControl7 = diameter*[imageArray count]+margin*([imageArray count]-1);
    float left_spacePageControl7 = (self.contentView.frame.size.width-width_spacePageControl7)/2.0;
    if(!self.spacePageControl7)
    {
        self.spacePageControl7 = [[SMPageControl alloc]init];
        [self.contentView addSubview:self.spacePageControl7];
    }
    self.spacePageControl7.numberOfPages = [imageArray count];
    self.spacePageControl7.indicatorMargin = margin;//水平间距
    self.spacePageControl7.indicatorDiameter = diameter;//直径
    [self.spacePageControl7 setFrame:CGRectMake(left_spacePageControl7, self.contentView.frame.size.height-height_spacePageControl7-bottom_spacePageControl7, width_spacePageControl7, height_spacePageControl7)];
    [self.spacePageControl7 setPageIndicatorImage:[UIImage imageNamed:@"mainpage_shuffings_unselected.png"]];
    [self.spacePageControl7 setCurrentPageIndicatorImage:[UIImage imageNamed:@"mainpage_shuffings_selected.png"]];
    self.spacePageControl7.userInteractionEnabled = NO;
...
-(void)fillScrollViewWithImageArray:(NSArray *)imageArray
{
    ...
    _currImageView.frame = CGRectMake(WIDTH_shuF, 0, WIDTH_shuF, HEIGHT_shuF);
...
#pragma mark 图片点击事件
-(void)imageClick {
    if(self.delegate&&[self.delegate respondsToSelector:@selector(phShuffingView:clickimageViewIndex:)])
    {
        [self.delegate phShuffingView:self clickimageViewIndex:self.currIndex];
    }
}
...
- (void)pauseScroll {
    ...
    self.spacePageControl7.currentPage = self.currIndex;
